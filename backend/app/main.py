...
async def lifespan(app: FastAPI):
    async with engine.begin() as conn:
        ...
        await conn.execute(text("ALTER TABLE employees ADD COLUMN IF NOT EXISTS payroll_compensation_mode VARCHAR DEFAULT 'hourly'"))
        await conn.execute(text("ALTER TABLE employees ADD COLUMN IF NOT EXISTS perdiem_mode VARCHAR DEFAULT ''"))
        await conn.execute(text("ALTER TABLE employees ADD COLUMN IF NOT EXISTS perdiem_threshold_hours DOUBLE PRECISION DEFAULT 7"))
        ...
        await conn.execute(text("UPDATE employees SET payroll_compensation_mode = 'hourly' WHERE payroll_compensation_mode IS NULL OR payroll_compensation_mode = ''"))
        await conn.execute(text("UPDATE employees SET perdiem_mode = '' WHERE perdiem_mode IS NULL"))
        await conn.execute(text("UPDATE employees SET perdiem_threshold_hours = 7 WHERE perdiem_threshold_hours IS NULL OR perdiem_threshold_hours <= 0"))
