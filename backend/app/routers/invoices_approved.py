"""Approved schedule invoice generation routes."""
...
async def generate_invoice_from_approved_schedules(
    data: dict,
    ...
    approval_result = await db.execute(
        select(ScheduleApproval).where(
            ...
            ScheduleApproval.week_start == ps,
            ScheduleApproval.week_end == pe,
    ...
    meta_result = await db.execute(
        select(ScheduleApprovalMeta).where(ScheduleApprovalMeta.approval_id == approval.id)
    )
    ...
    if abs(approved_hours - raw_total_hours) > 0.009:
        ...
        extra_lines.append({
            "description": f"Ajustement heures approuvées ({approved_hours:.2f}h approuvées vs {raw_total_hours:.2f}h planifiées)",
            "amount": round(delta_hours * rate, 2),
            "hours_delta": delta_hours,
            "rate": rate,
            "type": "approved_hours_adjustment",
        })
    ...
    invoice = Invoice(
        ...
        notes=(
            f"Facture approuvée générée depuis l'horaire validé. "
            f"Heures approuvées: {approved_hours:.2f}h. Heures planifiées: {raw_total_hours:.2f}h."
        ),
    )
    ...
    approval_atts_r = await db.execute(
        select(ScheduleApprovalAttachment)
        .where(ScheduleApprovalAttachment.approval_id == approval.id)
        .order_by(ScheduleApprovalAttachment.created_at)
    )
    ...
    db.add(InvoiceAuditLog(
        ...
        details=(
            f"Facture approuvée générée — {employee.name} / {client_name} / {ps} → {pe} / "
            f"{approved_hours:.2f}h approuvées / {raw_total_hours:.2f}h planifiées"
        ),
    ))
    await db.commit()
