...
async def _get_meta(db: AsyncSession, approval_id: int):
    result = await db.execute(
        select(ScheduleApprovalMeta).where(ScheduleApprovalMeta.approval_id == approval_id)
    )
    return result.scalar_one_or_none()
...
async def _upsert_meta(db: AsyncSession, approval_id: int, approved_hours: float, approved_shift_count: int, week_total_hours: float):
    ...
    if not meta:
        meta = ScheduleApprovalMeta(
            approval_id=approval_id,
            approved_hours=approved_hours,
            approved_shift_count=approved_shift_count,
...
async def _serialize_approval(db: AsyncSession, approval: ScheduleApproval):
    ...
    att_count = await db.execute(
        select(func.count(ScheduleApprovalAttachment.id)).where(ScheduleApprovalAttachment.approval_id == approval.id)
    )
    return {
        "id": approval.id,
        ...
        "week_end": str(approval.week_end),
        "status": approval.status,
        "approved_by": approval.approved_by,
        "approved_at": approval.approved_at.isoformat() if approval.approved_at else None,
        "notes": approval.notes,
        "approved_hours": round((meta.approved_hours if meta else 0) or 0, 2),
        "approved_shift_count": (meta.approved_shift_count if meta else 0) or 0,
        "week_total_hours": round((meta.week_total_hours if meta else 0) or 0, 2),
        "attachment_count": att_count.scalar() or 0,
    }
...
async def review_week(data: dict, db: AsyncSession = Depends(get_db), user=Depends(require_admin)):
    ...
    if not approval:
        approval = ScheduleApproval(
            ...
            week_start=ws,
            week_end=we,
            approved_by=getattr(user, "email", "admin"),
            status="pending",
            notes=notes,
        )
        db.add(approval)
...
async def list_approvals(
    ...
    for approval in approvals:
        out.append(await _serialize_approval(db, approval))
    return out


@router.post("/approvals/{approval_id}/attachments")
async def upload_approval_attachment(
    ...
    import uuid
    att = ScheduleApprovalAttachment(
        approval_id=approval_id,
...
@router.get("/approvals/{approval_id}/attachments")
async def list_approval_attachments(approval_id: int, db: AsyncSession = Depends(get_db), user=Depends(require_admin)):
    result = await db.execute(
        select(ScheduleApprovalAttachment)
        .where(ScheduleApprovalAttachment.approval_id == approval_id)
        .order_by(ScheduleApprovalAttachment.created_at.desc())
    )
    return [
        {
            "id": a.id,
...
@router.get("/approvals/{approval_id}/attachments/{att_id}")
async def download_approval_attachment(approval_id: int, att_id: int, db: AsyncSession = Depends(get_db), user=Depends(require_admin)):
    result = await db.execute(
        select(ScheduleApprovalAttachment).where(
            ScheduleApprovalAttachment.id == att_id,
            ScheduleApprovalAttachment.approval_id == approval_id,
        )
    )
...
@router.delete("/approvals/{approval_id}/attachments/{att_id}")
async def delete_approval_attachment(approval_id: int, att_id: int, db: AsyncSession = Depends(get_db), user=Depends(require_admin)):
    result = await db.execute(
        select(ScheduleApprovalAttachment).where(
            ScheduleApprovalAttachment.id == att_id,
            ScheduleApprovalAttachment.approval_id == approval_id,
