...
export default function InvoicesPage() {
  ...
  const loadClients = useCallback(async () => {
    try {
      setClients(await apiFetch('/clients'));
    } catch (_) {}
  }, []);

  const loadEmployees = useCallback(async () => {
    try {
      setEmployees(await apiFetch('/employees'));
    } catch (_) {}
  }, []);
  ...
  const bulkDelete = async () => {
    ...
    try {
      const res = await apiFetch('/invoices/bulk-delete', {
        method: 'POST',
      ...
      setSuccess(
        skippedCount
          ? `${deletedCount} facture(s) supprimée(s), ${skippedCount} ignorée(s)`
  ...
  const bulkValidate = async () => {
    ...
    if (!ids.length) return;
    try {
      const res = await apiFetch('/invoices/bulk/validate', {
        method: 'POST',
        body: JSON.stringify(ids),
      });
  ...
  const bulkSend = async () => {
    ...
    if (!ids.length) return;
    try {
      const res = await apiFetch('/invoices/bulk/send', {
        method: 'POST',
        body: JSON.stringify(ids),
      });
  ...
  return (
    <div style={S.page}>
      {success && (
        <div style={{ position: 'fixed', top: 16, right: 16, zIndex: 2000, background: '#28A745', color: '#fff', padding: '12px 20px', borderRadius: 8, boxShadow: '0 4px 20px rgba(0,0,0,0.2)', fontSize: 13, fontWeight: 600, maxWidth: 420 }}>
          ✓ {success}
        </div>
      )}
      {error && (
        <div style={{ position: 'fixed', top: 16, right: 16, zIndex: 2000, background: '#DC3545', color: '#fff', padding: '12px 20px', borderRadius: 8, boxShadow: '0 4px 20px rgba(0,0,0,0.2)', fontSize: 13, fontWeight: 600, maxWidth: 420 }}>
          ✗ {error}
        </div>
      )}
        ...
        <div>
          <div style={{ ...S.flexBetween, marginBottom: 16 }}>
            ...
            <div style={S.flexRow}>
              ...
              <button style={S.btn('primary')} onClick={() => setActiveTab('generate')}>
                + Générer factures
              </button>
            </div>
          </div>
                ...
                <tbody>
                  {invoices.map((inv, i) => (
                    <tr
                      ...
                      onMouseEnter={(e) => {
                        e.currentTarget.style.background = '#e8f4f6';
                      }}
                      onMouseLeave={(e) => {
      ...
      {activeTab === 'generate' && (
        <div style={S.card}>
          ...
          <p style={{ fontSize: 13, color: '#6C757D', marginBottom: 16 }}>
            Génère automatiquement 1 facture par employé par client pour la période sélectionnée.
          </p>
      ...
      {activeTab === 'detail' && selectedInvoice && (
        <InvoiceDetail
          ...
          onRefresh={refreshSelectedInvoice}
...
function InvoiceDetail({ invoice: inv, onBack, onRefresh, onStatusChange, onMarkPaid, onDuplicate, onDelete, onPdf, onEmail, setError, setSuccess, loadInvoices, loadStats }) {
  ...
  return (
    <div>
      ...
      {detailTab === 'lines' && (
        <div style={S.card}>
          {(inv.lines || []).length > 0 ? (
                ...
                <thead>
                  <tr>
                    {['Date', 'Début', 'Fin', 'Pause', 'Heures', 'Taux', 'Services', 'Garde', 'Rappel'].map((h, i) => (
                      <th key={h} style={{ ...S.th, ...(i >= 4 ? { textAlign: 'right' } : {}), ...(i === 0 ? { borderRadius: '8px 0 0 0' } : i === 8 ? { borderRadius: '0 8px 0 0' } : {}) }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {inv.lines.map((l, i) => (
                    <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa' }}>
                      ...
                      <td style={S.td}>{l.pause_min ? `${l.pause_min} min` : '—'}</td>
                      <td style={{ ...S.td, textAlign: 'right' }}>{l.hours?.toFixed?.(2) ?? l.hours ?? '0.00'}</td>
            ...
            <div style={S.empty}>Aucune ligne de service</div>
          )}
          ...
          {(inv.extra_lines || []).length > 0 && (
            <>
              <h4 style={{ ...S.sectionTitle, fontSize: 13, marginTop: 20 }}>Ajustements</h4>
              <table style={S.table}>
                <thead>
                  <tr>
                    {['Description', 'Montant'].map((h, i) => (
                      <th key={h} style={{ ...S.th, ...(i === 1 ? { textAlign: 'right' } : {}) }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {inv.extra_lines.map((e, i) => (
                    <tr key={i}>
                      <td style={S.td}>{e.description}</td>
                      <td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(e.amount)}</td>
                    </tr>
          ...
          <div style={{ marginTop: 20, borderTop: '2px solid #e9ecef', paddingTop: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <div style={{ minWidth: 300 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: 14, fontWeight: 600 }}>
                ...
                {inv.include_tax ? (
                  <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 13, color: '#6C757D' }}>
                      <span>TPS (5%)</span>
                      <span>{fmt(inv.tps)}</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 13, color: '#6C757D' }}>
                      <span>TVQ (9.975%)</span>
                      <span>{fmt(inv.tvq)}</span>
                    </div>
                  </>
                ) : (
        ...
        <div style={S.card}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}>
              ...
              <div>
                <label style={{ ...S.btn('primary', 'sm'), cursor: 'pointer', display: 'inline-block' }}>
                  ...
                  <input type="file" accept=".pdf,.jpg,.jpeg,.png,.gif" style={{ display: 'none' }} onChange={handleAttachmentUpload} disabled={attUploading} />
                </label>
              </div>
            </div>
          </div>
            ...
            <div>
              {attachments.map((att) => (
                <div key={att.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid #f0f0f0' }}>
                  <span style={{ fontSize: 24 }}>{att.file_type === 'pdf' ? '📄' : '🖼️'}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{att.filename}</div>
                    <div style={{ fontSize: 11, color: '#6C757D' }}>
                      {att.category} — {fmtDate(att.created_at)}
                      {att.description ? ` — ${att.description}` : ''}
                    </div>
                  </div>
                  <button style={{ ...S.btn('outline', 'sm'), padding: '4px 8px' }} onClick={() => openAttachment(att.id)}>👁️</button>
                  <button style={{ ...S.btn('danger', 'sm'), padding: '4px 8px' }} onClick={() => removeAttachment(att.id)}>🗑️</button>
                </div>
              ))}
            </div>
          )}
          ...
          {attachments.length > 0 && (
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid #e9ecef' }}>
              <button style={S.btn('primary', 'md')} onClick={combinedPdf}>📥 PDF facture + pièces jointes</button>
            </div>
          )}
        </div>
      )}
      ...
      {detailTab === 'audit' && (
        <div style={S.card}>
          <h4 style={{ ...S.sectionTitle, margin: '0 0 12px 0' }}>📜 Historique</h4>
      ...
      <Modal open={showPayment} onClose={() => setShowPayment(false)} title="Enregistrer un paiement">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            ...
            <input type="number" step="0.01" style={{ ...S.input, width: '100%' }} value={payAmount} onChange={(e) => setPayAmount(e.target.value)} placeholder={`Max: ${inv.balance_due?.toFixed?.(2) || inv.balance_due}`} />
          </div>
          <div>
            ...
            <thead>
              <tr>
                {['Client', 'Facturé', 'Payé', 'En cours', 'En retard', 'Nb factures', ''].map((h, i) => (
                  <th key={h} style={{ ...S.th, ...(i >= 1 && i <= 4 ? { textAlign: 'right' } : {}), ...(i === 0 ? { borderRadius: '8px 0 0 0' } : i === 6 ? { borderRadius: '0 8px 0 0' } : {}) }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {reportData.map((c, i) => (
                <React.Fragment key={c.client_id}>
                  <tr
                    style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa', cursor: 'pointer' }}
                    onClick={() => setExpandedClient(expandedClient === c.client_id ? null : c.client_id)}
                  >
                    <td style={{ ...S.td, fontWeight: 600 }}>{expandedClient === c.client_id ? '▼' : '▶'} {c.client_name}</td>
                    ...
                    <td style={{ ...S.td, textAlign: 'right', color: c.total_overdue > 0 ? '#DC3545' : '#6C757D' }}>{fmt(c.total_overdue)}</td>
                  ...
                  {expandedClient === c.client_id && (c.invoices || []).map((inv) => (
                    <tr key={inv.id} style={{ background: '#f0f8f9' }}>
                      <td style={{ ...S.td, paddingLeft: 32 }}>
                        <span style={{ color: '#2A7B88', cursor: 'pointer', fontWeight: 600, textDecoration: 'underline' }} onClick={() => onOpenInvoice(inv.id)}>
                          {inv.number}
                        </span>
                      ...
                      <td style={S.td}>
                        {inv.balance_due > 0 && !['draft', 'cancelled'].includes(inv.status) && (
          ...
          <tbody>
            {reportData.map((e, i) => (
              <tr key={e.employee_id} style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa' }}>
                ...
                <td style={{ ...S.td, textAlign: 'right' }}>{e.total_hours?.toFixed?.(1) ?? e.total_hours}h</td>
...
function CreditNotesTab({ creditNotes, onRefresh, setError, setSuccess, clients, invoices }) {
  ...
  const toggleAllCreditNotes = () => {
    ...
    setSelectedCreditNotes((prev) => (
      prev.size === creditNotes.length ? new Set() : new Set(creditNotes.map((cn) => cn.id))
    ));
  };
    ...
    try {
      await apiFetch('/invoices/credit-notes', {
        ...
        body: JSON.stringify({
          ...
          client_id: cnClientId ? parseInt(cnClientId, 10) : null,
          reason: cnReason,
  ...
  const bulkDelete = async () => {
    ...
    try {
      const res = await apiFetch('/invoices/credit-notes/bulk-delete', {
        method: 'POST',
        body: JSON.stringify(ids),
      });
      const deletedCount = res?.deleted?.length || res?.count || 0;
      ...
      setSuccess(
        skippedCount
          ? `${deletedCount} note(s) supprimée(s), ${skippedCount} ignorée(s)`
          : `${deletedCount} note(s) supprimée(s)`
      );
    ...
    <div>
      <div style={{ ...S.flexBetween, marginBottom: 16 }}>
        ...
        <div style={S.flexRow}>
          {selectedCreditNotes.size > 0 && (
            <>
              <span style={{ fontSize: 12, color: '#6C757D' }}>{selectedCreditNotes.size} sélectionnée(s)</span>
              <button style={S.btn('danger')} onClick={bulkDelete}>Supprimer</button>
            </>
          )}
      ...
      {creditNotes.length === 0 ? (
        ...
        <table style={S.table}>
          <thead>
            <tr>
              <th style={{ ...S.th, width: 30, borderRadius: '8px 0 0 0' }}>
                <input type="checkbox" checked={selectedCreditNotes.size === creditNotes.length && creditNotes.length > 0} onChange={toggleAllCreditNotes} />
              </th>
              {['Numéro', 'Date', 'Client', 'Facture réf.', 'Raison', 'Montant', 'Total', 'Statut', ''].map((h, i) => (
                <th key={h} style={{ ...S.th, ...(i >= 5 && i <= 6 ? { textAlign: 'right' } : {}), ...(i === 8 ? { borderRadius: '0 8px 0 0' } : {}) }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {creditNotes.map((cn, i) => (
              <tr key={cn.id} style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa' }}>
                <td style={S.td}>
                  <input type="checkbox" checked={selectedCreditNotes.has(cn.id)} onChange={() => toggleCreditNote(cn.id)} />
                </td>
                ...
                <td style={{ ...S.td, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cn.reason}</td>
