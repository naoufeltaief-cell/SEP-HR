    ...
    <div style={S.page}>
      ...
      {activeTab === 'list' && <div>
        <div style={{ ...S.flexBetween, marginBottom: 16 }}>
          ...
          <div style={S.flexRow}>
            {selected.size > 0 && <><span style={{ fontSize: 12, color: '#6C757D' }}>{selected.size} sélectionnée(s)</span><button style={S.btn('primary')} onClick={bulkValidate}>Valider</button><button style={S.btn('outline')} onClick={bulkSend}>Envoyer</button></>}
...
function InvoiceDetail({ invoice: inv, onBack, onRefresh, onStatusChange, onMarkPaid, onDuplicate, onDelete, onPdf, onEmail, setError, setSuccess, loadInvoices, loadStats }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}><div style={S.statCard('#2A7B88')}><div style={{ fontSize: 11, color: '#6C757D', fontWeight: 600 }}>CLIENT</div><div style={{ fontSize: 14, fontWeight: 700 }}>{inv.client_name}</div><div style={{ fontSize: 11, color: '#6C757D' }}>{inv.client_email}</div></div><div style={S.statCard('#6C757D')}><div style={{ fontSize: 11, color: '#6C757D', fontWeight: 600 }}>RESSOURCE</div><div style={{ fontSize: 14, fontWeight: 700 }}>{inv.employee_name}</div><div style={{ fontSize: 11, color: '#6C757D' }}>{inv.employee_title}</div></div><div style={S.statCard('#2A7B88')}><div style={{ fontSize: 11, color: '#6C757D', fontWeight: 600 }}>PÉRIODE</div><div style={{ fontSize: 13, fontWeight: 600 }}>{fmtDate(inv.period_start)} → {fmtDate(inv.period_end)}</div></div><div style={S.statCard(inv.balance_due > 0 ? '#DC3545' : '#28A745')}><div style={{ fontSize: 11, color: '#6C757D', fontWeight: 600 }}>SOLDE DÛ</div><div style={{ fontSize: 22, fontWeight: 700, color: inv.balance_due > 0 ? '#DC3545' : '#28A745' }}>{fmt(inv.balance_due)}</div></div></div>
      ...
      {detailTab === 'lines' && (
        <div style={S.card}>
          {(inv.lines || []).length > 0 && <><h4 style={{ ...S.sectionTitle, fontSize: 13 }}>Services</h4><div style={{ overflowX: 'auto' }}><table style={S.table}><thead><tr>{['Date', 'Début', 'Fin', 'Pause', 'Heures', 'Taux', 'Services', 'Garde', 'Rappel'].map((h, i) => <th key={h} style={{ ...S.th, fontSize: 10, ...(i === 0 ? { borderRadius: '6px 0 0 0' } : i === 8 ? { borderRadius: '0 6px 0 0' } : {}), ...(i >= 4 ? { textAlign: 'right' } : {}) }}>{h}</th>)}</tr></thead><tbody>{inv.lines.map((l, i) => <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa' }}><td style={S.td}>{l.date?.substring(0, 10)}</td><td style={S.td}>{l.start}</td><td style={S.td}>{l.end}</td><td style={S.td}>{l.pause_min} min</td><td style={{ ...S.td, textAlign: 'right' }}>{l.hours?.toFixed(2)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(l.rate)}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(l.service_amount)}</td><td style={{ ...S.td, textAlign: 'right' }}>{l.garde_amount ? fmt(l.garde_amount) : '—'}</td><td style={{ ...S.td, textAlign: 'right' }}>{l.rappel_amount ? fmt(l.rappel_amount) : '—'}</td></tr>)}</tbody></table></div></>}
          {(inv.accommodation_lines || []).length > 0 && <><h4 style={{ ...S.sectionTitle, fontSize: 13, marginTop: 20 }}>Hébergement</h4><table style={S.table}><thead><tr>{['Employé', 'Période', 'Jours', 'Coût/jour', 'Montant'].map((h, i) => <th key={h} style={{ ...S.th, fontSize: 10, ...(i >= 2 ? { textAlign: 'right' } : {}) }}>{h}</th>)}</tr></thead><tbody>{inv.accommodation_lines.map((a, i) => <tr key={i}><td style={S.td}>{a.employee}</td><td style={S.td}>{a.period}</td><td style={{ ...S.td, textAlign: 'right' }}>{a.days}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(a.cost_per_day)}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(a.amount)}</td></tr>)}</tbody></table></>}
          {(inv.expense_lines || []).length > 0 && <><h4 style={{ ...S.sectionTitle, fontSize: 13, marginTop: 20 }}>Frais</h4><table style={S.table}><thead><tr>{['Description', 'Quantité', 'Taux', 'Montant'].map((h, i) => <th key={h} style={{ ...S.th, fontSize: 10, ...(i >= 1 ? { textAlign: 'right' } : {}) }}>{h}</th>)}</tr></thead><tbody>{inv.expense_lines.map((e, i) => <tr key={i}><td style={S.td}>{e.description}</td><td style={{ ...S.td, textAlign: 'right' }}>{e.quantity?.toFixed(2)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(e.rate)}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(e.amount)}</td></tr>)}</tbody></table></>}
          {(inv.extra_lines || []).length > 0 && <><h4 style={{ ...S.sectionTitle, fontSize: 13, marginTop: 20 }}>Ajustements</h4><table style={S.table}><thead><tr>{['Description', 'Quantité', 'Taux', 'Montant'].map((h, i) => <th key={h} style={{ ...S.th, fontSize: 10, ...(i >= 1 ? { textAlign: 'right' } : {}) }}>{h}</th>)}</tr></thead><tbody>{inv.extra_lines.map((e, i) => <tr key={i}><td style={S.td}>{e.description}</td><td style={{ ...S.td, textAlign: 'right' }}>{Number(e.quantity || 0).toFixed(2)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(e.rate)}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(e.amount)}</td></tr>)}</tbody></table></>}
          <div style={{ marginTop: 20, borderTop: '2px solid #e9ecef', paddingTop: 16 }}><div style={{ display: 'flex', justifyContent: 'flex-end' }}><div style={{ minWidth: 300 }}>{[['Services', inv.subtotal_services], inv.subtotal_garde && ['Garde', inv.subtotal_garde], inv.subtotal_rappel && ['Rappel', inv.subtotal_rappel], inv.subtotal_accom && ['Hébergement', inv.subtotal_accom], inv.subtotal_deplacement && ['Déplacement', inv.subtotal_deplacement], inv.subtotal_km && ['Kilométrage', inv.subtotal_km], inv.subtotal_autres_frais && ['Autres frais', inv.subtotal_autres_frais]].filter(Boolean).map(([label, val]) => <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 13 }}><span style={{ color: '#6C757D' }}>{label}</span><span>{fmt(val)}</span></div>)}<div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: 14, fontWeight: 600, borderTop: '1px solid #dee2e6', marginTop: 6 }}><span>Sous-total</span><span>{fmt(inv.subtotal)}</span></div>{inv.include_tax && <><div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 13, color: '#6C757D' }}><span>TPS (5%)</span><span>{fmt(inv.tps)}</span></div><div style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 13, color: '#6C757D' }}><span>TVQ (9.975%)</span><span>{fmt(inv.tvq)}</span></div></>}{!inv.include_tax && <div style={{ padding: '3px 0', fontSize: 12, color: '#28A745', fontStyle: 'italic' }}>Exempté de taxes</div>}<div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 12px', fontSize: 18, fontWeight: 700, background: '#2A7B88', color: '#fff', borderRadius: 8, marginTop: 8 }}><span>TOTAL</span><span>{fmt(inv.total)}</span></div>{inv.amount_paid > 0 && <><div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: 13, marginTop: 8, color: '#28A745' }}><span>Payé</span><span>{fmt(inv.amount_paid)}</span></div><div style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: 15, fontWeight: 700, color: inv.balance_due > 0 ? '#DC3545' : '#28A745' }}><span>Solde dû</span><span>{fmt(inv.balance_due)}</span></div></>}</div></div></div>
        </div>
      )}
      {detailTab === 'payments' && <div style={S.card}><div style={S.flexBetween}><h4 style={{ ...S.sectionTitle, margin: 0 }}>💰 Paiements</h4>{!['draft', 'cancelled', 'paid'].includes(inv.status) && <button style={S.btn('success')} onClick={() => setShowPayment(true)}>+ Nouveau paiement</button>}</div>{(inv.payments || []).length === 0 ? <div style={S.empty}>Aucun paiement enregistré</div> : <table style={{ ...S.table, marginTop: 12 }}><thead><tr>{['Date', 'Montant', 'Méthode', 'Référence', 'Notes', ''].map((h, i) => <th key={h} style={{ ...S.th, fontSize: 10 }}>{h}</th>)}</tr></thead><tbody>{inv.payments.map(p => <tr key={p.id}><td style={S.td}>{fmtDate(p.date)}</td><td style={{ ...S.td, fontWeight: 600, color: '#28A745' }}>{fmt(p.amount)}</td><td style={S.td}>{p.method}</td><td style={S.td}>{p.reference || '—'}</td><td style={S.td}>{p.notes || '—'}</td><td style={S.td}><button style={S.btn('danger', 'sm')} onClick={() => deletePayment(p.id)}>🗑</button></td></tr>)}</tbody></table>}</div>}
      {detailTab === 'attachments' && <div style={S.card}><h4 style={{ ...S.sectionTitle, margin: '0 0 12px 0' }}>📎 Pièces jointes</h4><div style={{ background: '#f8f9fa', borderRadius: 8, padding: 14, marginBottom: 16, border: '1px dashed #ced4da' }}><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-end' }}><div style={{ flex: 1, minWidth: 140 }}><label style={{ fontSize: 11, fontWeight: 600, display: 'block', marginBottom: 4 }}>Catégorie</label><select style={{ ...S.select, width: '100%' }} value={attCategory} onChange={e => setAttCategory(e.target.value)}><option value="hebergement">Hébergement</option><option value="deplacement">Déplacement</option><option value="kilometrage">Kilométrage</option><option value="autre">Autre</option></select></div><div style={{ flex: 2, minWidth: 180 }}><label style={{ fontSize: 11, fontWeight: 600, display: 'block', marginBottom: 4 }}>Description (opt.)</label><input style={{ ...S.input, width: '100%' }} value={attDescription} onChange={e => setAttDescription(e.target.value)} placeholder="Reçu hôtel, facture..." /></div><div><label style={{ ...S.btn('primary', 'sm'), cursor: 'pointer', display: 'inline-block' }}>{attUploading ? '⏳ Upload...' : '📤 Ajouter fichier'}<input type="file" accept=".pdf,.jpg,.jpeg,.png,.gif" style={{ display: 'none' }} onChange={handleAttUpload} disabled={attUploading} /></label></div></div><div style={{ fontSize: 11, color: '#6C757D', marginTop: 6 }}>Formats acceptés : PDF, JPG, PNG, GIF — Max 10 MB</div></div>{attLoading ? <div style={S.empty}>Chargement...</div> : attachments.length === 0 ? <div style={S.empty}>Aucune pièce jointe</div> : <div>{attachments.map(att => <div key={att.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid #f0f0f0' }}><span style={{ fontSize: 24 }}>{typeIcons[att.file_type] || '📄'}</span><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontWeight: 600, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{att.filename}</div><div style={{ fontSize: 11, color: '#6C757D' }}><span style={{ background: '#e9ecef', borderRadius: 4, padding: '1px 6px', marginRight: 6 }}>{catLabels[att.category] || att.category}</span>{fmtSize(att.file_size)} — {fmtDate(att.created_at?.split('T')[0])}{att.description && <span style={{ marginLeft: 6 }}>— {att.description}</span>}</div></div><button style={{ ...S.btn('outline', 'sm'), padding: '4px 8px' }} onClick={() => viewAttachment(att.id)} title="Voir">👁️</button><button style={{ ...S.btn('danger', 'sm'), padding: '4px 8px' }} onClick={() => deleteAttachment(att.id)} title="Supprimer">🗑️</button></div>)}</div>}{attachments.length > 0 && <div style={{ marginTop: 16, paddingTop: 12, borderTop: '1px solid #e9ecef' }}><button style={S.btn('primary', 'md')} onClick={downloadPdfWithAttachments}>📥 Télécharger PDF facture + pièces jointes</button></div>}</div>}
      {detailTab === 'audit' && <div style={S.card}><h4 style={{ ...S.sectionTitle, margin: '0 0 12px 0' }}>📜 Historique des modifications</h4>{(inv.audit_logs || []).length === 0 ? <div style={S.empty}>Aucun événement</div> : <div>{inv.audit_logs.map(log => <div key={log.id} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}><div style={{ minWidth: 140, color: '#6C757D', fontSize: 11 }}>{fmtDateTime(log.created_at)}</div><div style={{ flex: 1 }}><span style={{ fontWeight: 600 }}>{log.action}</span>{log.old_status && log.new_status && <span style={{ marginLeft: 8 }}><StatusBadge status={log.old_status} /> → <StatusBadge status={log.new_status} /></span>}{log.details && <div style={{ color: '#6C757D', marginTop: 2, fontSize: 12 }}>{log.details}</div>}{log.user_email && <div style={{ color: '#adb5bd', fontSize: 11 }}>par {log.user_email}</div>}</div></div>)}</div>}</div>}
      <InvoiceEditModal open={showEdit} onClose={() => setShowEdit(false)} invoice={inv} onSave={saveInvoiceEdits} />
...
function ReportsTab({ reportData, reportType, onLoadReport, onOpenInvoice, onMarkPaid }) {
  ...
  return (
    <div>
      ...
      {!reportData ? <div style={S.empty}>Sélectionnez un rapport</div> : reportType === 'by-client' ? <div><table style={S.table}><thead><tr>{['Client', 'Facturé', 'Payé', 'En cours', 'En retard', 'Nb factures', ''].map((h, i) => <th key={h} style={{ ...S.th, ...(i >= 1 && i <= 4 ? { textAlign: 'right' } : {}), ...(i === 0 ? { borderRadius: '8px 0 0 0' } : i === 6 ? { borderRadius: '0 8px 0 0' } : {}) }}>{h}</th>)}</tr></thead><tbody>{reportData.map((c, i) => <React.Fragment key={c.client_id}><tr style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa', cursor: 'pointer' }} onClick={() => setExpandedClient(expandedClient === c.client_id ? null : c.client_id)} onMouseEnter={e => e.currentTarget.style.background = '#e8f4f6'} onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? '#fff' : '#f8f9fa'}><td style={{ ...S.td, fontWeight: 600 }}>{expandedClient === c.client_id ? '▼' : '▶'} {c.client_name}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(c.total_invoiced)}</td><td style={{ ...S.td, textAlign: 'right', color: '#28A745' }}>{fmt(c.total_paid)}</td><td style={{ ...S.td, textAlign: 'right', color: '#FFC107' }}>{fmt(c.total_outstanding)}</td><td style={{ ...S.td, textAlign: 'right', color: c.total_overdue > 0 ? '#DC3545' : '#6C757D', fontWeight: c.total_overdue > 0 ? 700 : 400 }}>{fmt(c.total_overdue)}</td><td style={{ ...S.td, textAlign: 'center' }}>{c.invoice_count}</td><td style={S.td}></td></tr>{expandedClient === c.client_id && (c.invoices || []).map(inv => <tr key={inv.id} style={{ background: '#f0f8f9' }}><td style={{ ...S.td, paddingLeft: 32 }}><span style={{ color: '#2A7B88', cursor: 'pointer', fontWeight: 600, textDecoration: 'underline' }} onClick={() => onOpenInvoice(inv.id)}>{inv.number}</span><span style={{ marginLeft: 8, fontSize: 11, color: '#6C757D' }}>{inv.employee_name}</span></td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(inv.total)}</td><td style={{ ...S.td, textAlign: 'right', color: '#28A745' }}>{fmt(inv.amount_paid)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(inv.balance_due)}</td><td style={S.td}></td><td style={S.td}><StatusBadge status={inv.status} /></td><td style={S.td}>{inv.balance_due > 0 && inv.status !== 'draft' && inv.status !== 'cancelled' && <button style={S.btn('success', 'sm')} onClick={(e) => { e.stopPropagation(); onMarkPaid(inv.id); }}>💰 Payée</button>}</td></tr>)}</React.Fragment>)}</tbody></table>{reportData.length > 0 && <div style={{ display: 'flex', gap: 20, padding: '12px 16px', background: '#2A7B88', color: '#fff', borderRadius: '0 0 8px 8px', fontSize: 13, fontWeight: 600 }}><span>TOTAUX:</span><span>Facturé: {fmt(reportData.reduce((s, c) => s + c.total_invoiced, 0))}</span><span>Payé: {fmt(reportData.reduce((s, c) => s + c.total_paid, 0))}</span><span>En cours: {fmt(reportData.reduce((s, c) => s + c.total_outstanding, 0))}</span><span>En retard: {fmt(reportData.reduce((s, c) => s + c.total_overdue, 0))}</span></div>}</div> : reportType === 'by-employee' ? <table style={S.table}><thead><tr>{['Employé', 'Titre', 'Facturé', 'Heures', 'Nb factures'].map((h, i) => <th key={h} style={{ ...S.th, ...(i >= 2 ? { textAlign: 'right' } : {}) }}>{h}</th>)}</tr></thead><tbody>{reportData.map((e, i) => <tr key={e.employee_id} style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa' }}><td style={{ ...S.td, fontWeight: 600 }}>{e.employee_name}</td><td style={S.td}>{e.employee_title}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(e.total_invoiced)}</td><td style={{ ...S.td, textAlign: 'right' }}>{e.total_hours?.toFixed(1)}h</td><td style={{ ...S.td, textAlign: 'right' }}>{e.invoice_count}</td></tr>)}</tbody></table> : <table style={S.table}><thead><tr>{['Mois', 'Services', 'Garde', 'Rappel', 'Héberg.', 'Frais', 'Sous-total', 'Taxes', 'Total'].map((h, i) => <th key={h} style={{ ...S.th, ...(i >= 1 ? { textAlign: 'right' } : {}) }}>{h}</th>)}</tr></thead><tbody>{reportData.map((m, i) => <tr key={m.period} style={{ background: i % 2 === 0 ? '#fff' : '#f8f9fa' }}><td style={{ ...S.td, fontWeight: 600 }}>{m.period}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(m.services)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(m.garde)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(m.rappel)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(m.accommodation)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(m.expenses)}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{fmt(m.subtotal)}</td><td style={{ ...S.td, textAlign: 'right' }}>{fmt(m.taxes)}</td><td style={{ ...S.td, textAlign: 'right', fontWeight: 700, color: '#2A7B88' }}>{fmt(m.total)}</td></tr>)}</tbody>{reportData.length > 0 && <tfoot><tr style={{ background: '#2A7B88' }}><td style={{ ...S.td, color: '#fff', fontWeight: 700 }}>TOTAL</td>{['services', 'garde', 'rappel', 'accommodation', 'expenses', 'subtotal', 'taxes', 'total'].map(k => <td key={k} style={{ ...S.td, textAlign: 'right', color: '#fff', fontWeight: 700 }}>{fmt(reportData.reduce((s, m) => s + (m[k] || 0), 0))}</td>)}</tr></tfoot>}</table>}
    </div>
  );
}
