...
function ApprovalPanel({ employee, client, shifts, allEmployeeSchedules, reviewDraft, setReviewDraft, currentReview, currentInvoice, reviewAttachments, onSave, onApprove, onRevoke, onGenerateInvoice, onUpload, onDeleteAttachment, onOpenAttachment, onGoInvoices, onRefreshParent, toast }) {
  ...
  const updateEditableShift = (id, field, value) => setEditableShifts(prev => prev.map(s => { if (s.id !== id) return s; const next = { ...s, [field]: value }; if (field === 'start' || field === 'end' || field === 'pause_minutes') { next.pause = pauseMinutesToHours(next.pause_minutes); next.hours = calcHours(next.start, next.end, Number(next.pause || 0)); } return next; }));
  const addQuickShiftRow = () => { const baseDate = shifts?.[0]?.date || fmtISO(new Date()); setEditableShifts(prev => [...prev, { id: `new-${Date.now()}-${prev.length}`, is_new: true, employee_id: employee.id, client_id: client?.id || null, date: baseDate, start: '07:00', end: '15:00', pause: 0, pause_minutes: 0, hours: 8, km: 0, deplacement: 0, other_dep: 0, notes: '', billable_rate: Number(employee?.rate || 0), status: 'published', location: '' }]); };
  ...
  const removeShiftLine = async (shift) => { if (shift.is_new) { setEditableShifts(prev => prev.filter(s => s.id !== shift.id)); return; } const snapshot = editableShifts.map(s => ({ ...s })); try { setSavingShiftId(shift.id); isSavingRef.current = true; setEditableShifts(prev => prev.filter(s => s.id !== shift.id)); await api.deleteSchedule(shift.id); toast?.('Quart supprimé'); await onRefreshParent?.(); } catch (err) { setEditableShifts(snapshot); toast?.('Erreur: ' + (err.message || 'Échec de la suppression')); } finally { isSavingRef.current = false; setSavingShiftId(null); } };
