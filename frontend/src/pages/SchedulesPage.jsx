...
export default function SchedulesPage({ toast, onNavigate }) {
  ...
  const saveShift = async () => {
    try {
      const d = modal.data;
      const payload = { employee_id: Number(d.employeeId), date: d.date, start: normalizeTimeForInput(d.start), end: normalizeTimeForInput(d.end), pause: Number(d.pause || 0), hours: Number(d.hours || 0), location: d.location || '', client_id: d.clientId ? Number(d.clientId) : null, km: Number(d.km || 0), deplacement: Number(d.deplacement || 0), autre_dep: Number(d.autreDep || 0), notes: d.notes || '', billable_rate: 0, status: 'published' };
      if (modal.type === 'add') await api.createSchedule(payload);
      else await api.updateSchedule(d.id, payload);
      toast?.(modal.type === 'add' ? 'Quart ajouté' : 'Quart modifié');
      setModal(null);
      await reload();
  ...
  const toggleBillingPanel = async (empId, clientId, fallbackClientId = null) => {
    ...
    setCurrentReview(null); setCurrentInvoice(null); setReviewAttachments([]);
    ...
    try {
      ...
      setCurrentReview(review); setCurrentInvoice(invoice);
      if (review?.id) {
  ...
  const saveReview = async (empId, clientId, fallbackClientId = null, approve = false) => {
    ...
    try {
      ...
      if (approve && (!review || review.status !== 'approved')) review = { ...(review || {}), employee_id: empId, client_id: clientId, week_start: ws, status: 'approved' };
  ...
  const openReviewAttachment = async (attId) => {
    ...
    window.open(`${base}/schedule-reviews/${currentReview.id}/attachments/${attId}`, '_blank');
  };
  ...
  return <>
    <div className="page-header">
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button className="btn btn-outline btn-sm" onClick={() => api.publishAll().then(() => { toast?.('Quarts publiés'); reload(); }).catch(err => toast?.('Erreur: ' + err.message))}><Send size={13} /> Publier tout</button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button className={`btn btn-sm ${viewMode === 'week' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setViewMode('week')}>Semaine</button>
          <button className={`btn btn-sm ${viewMode === 'month' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setViewMode('month')}>Mois</button>
        </div>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        <div style={{ position: 'relative', maxWidth: 300 }}>
          <Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text3)' }} />
          <input className="input" style={{ paddingLeft: 32, padding: '7px 12px 7px 32px', fontSize: 12 }} placeholder="Rechercher un employé..." value={filterText} onChange={e => setFilterText(e.target.value)} />
        </div>
    ...
    <div className="schedule-grid">
      <table>
        <thead>
          <tr><th>Employé</th>{viewDates.map((d, i) => <th key={i}>{fmtDay(d)}</th>)}<th style={{ minWidth: 120, background: 'var(--brand-xl)' }}>Total</th></tr>
        </thead>
        <tbody>
          {activeEmps.map(e => {
            const periodShifts = schedules.filter(s => s.employee_id === e.id && viewISOs.includes(s.date));
            const totalHrs = periodShifts.reduce((sum, s) => sum + Number(s.hours || 0), 0);
            const totalKm = periodShifts.reduce((sum, s) => sum + Number(s.km || 0), 0);
            const totalDep = periodShifts.reduce((sum, s) => sum + Number(s.deplacement || 0) + Number(s.autre_dep || 0), 0);
            ...
            return <React.Fragment key={e.id}>
              <tr>
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }} onClick={() => onNavigate && onNavigate('employees')}>
                    <Avatar name={e.name} size={30} />
                    <div><div style={{ fontWeight: 600, fontSize: 12 }}>{e.name}</div><div style={{ fontSize: 10, color: 'var(--text3)' }}>{(e.position || '').slice(0, 24)}</div></div>
                  </div>
                </td>
                {viewDates.map((d, i) => {
                  const iso = fmtISO(d);
                  const shifts = schedules.filter(s => s.employee_id === e.id && s.date === iso);
                  return <td key={i} onDoubleClick={() => openAdd(e.id, iso)}>{shifts.map(s => <ShiftPill key={s.id} shift={s} onClick={() => openEdit(s)} />)}{shifts.length === 0 && <div onClick={() => openAdd(e.id, iso)} style={{ opacity: .25, textAlign: 'center', fontSize: 16, lineHeight: '30px', color: 'var(--brand)', cursor: 'pointer' }}>+</div>}</td>;
                })}
    ...
    <Modal open={!!modal} onClose={() => setModal(null)} title={modal?.type === 'edit' ? 'Modifier le quart' : 'Ajouter un quart'}>
      {modal && <div style={{ display: 'grid', gap: 10 }}>
        <div>
          <label>Employé</label>
          <select className="input" value={modal.data.employeeId} onChange={e => updateModalField('employeeId', e.target.value)}>
            <option value="">Choisir</option>
            {employees.map(emp => <option key={emp.id} value={emp.id}>{emp.name}</option>)}
          </select>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          <div><label>Date</label><input className="input" type="date" value={modal.data.date} onChange={e => updateModalField('date', e.target.value)} /></div>
          <div><label>Client</label><select className="input" value={modal.data.clientId} onChange={e => updateModalField('clientId', e.target.value)}><option value="">—</option>{clients.map(cl => <option key={cl.id} value={cl.id}>{cl.name}</option>)}</select></div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <div><label>Début</label><input className="input" type="time" value={modal.data.start} onChange={e => updateModalField('start', e.target.value)} /></div>
          <div><label>Fin</label><input className="input" type="time" value={modal.data.end} onChange={e => updateModalField('end', e.target.value)} /></div>
          <div><label>Pause min</label><input className="input" type="number" value={modal.data.pauseMinutes} onChange={e => updateModalField('pauseMinutes', e.target.value)} /></div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <div><label>Heures</label><input className="input" value={Number(modal.data.hours || 0).toFixed(2)} readOnly /></div>
          <div><label>KM</label><input className="input" type="number" value={modal.data.km} onChange={e => updateModalField('km', e.target.value)} /></div>
          <div><label>Déplacement</label><input className="input" type="number" step="0.01" value={modal.data.deplacement} onChange={e => updateModalField('deplacement', e.target.value)} /></div>
        </div>
        <div><label>Autre dépense</label><input className="input" type="number" step="0.01" value={modal.data.autreDep} onChange={e => updateModalField('autreDep', e.target.value)} /></div>
        <div><label>Lieu</label><input className="input" value={modal.data.location} onChange={e => updateModalField('location', e.target.value)} /></div>
        <div><label>Notes</label><input className="input" value={modal.data.notes} onChange={e => updateModalField('notes', e.target.value)} /></div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          <div>{modal.type === 'edit' && <button className="btn btn-outline btn-sm" onClick={deleteShift}><Trash2 size={14} /> Supprimer</button>}</div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-outline btn-sm" onClick={() => setModal(null)}>Annuler</button>
            <button className="btn btn-primary btn-sm" onClick={saveShift}>Sauvegarder</button>
          </div>
        </div>
      </div>}
    </Modal>
...
function ApprovalPanel({ employee, client, shifts, allEmployeeSchedules, reviewDraft, setReviewDraft, currentReview, currentInvoice, reviewAttachments, onSave, onApprove, onRevoke, onGenerateInvoice, onUpload, onDeleteAttachment, onOpenAttachment, onGoInvoices, onRefreshParent }) {
  ...
  useEffect(() => {
    setEditableShifts((shifts || []).map(s => ({ ...s, start: normalizeTimeForInput(s.start), end: normalizeTimeForInput(s.end), other_dep: s.autre_dep ?? s.other_dep ?? 0, pause_minutes: pauseHoursToMinutes(s.pause), is_new: false })));
  }, [shifts]);

  const loadAccommodations = useCallback(async () => {
    if (!employee?.id) { setAccommodations([]); return; }
    try {
  ...
  const updateEditableShift = (id, field, value) => setEditableShifts(prev => prev.map(s => {
    if (s.id !== id) return s;
  ...
  const addQuickShiftRow = () => {
    ...
    setEditableShifts(prev => [...prev, { id: `new-${Date.now()}-${prev.length}`, is_new: true, employee_id: employee.id, client_id: client?.id || null, date: baseDate, start: '07:00', end: '15:00', pause: 0, pause_minutes: 0, hours: 8, km: 0, deplacement: 0, other_dep: 0, notes: '', billable_rate: Number(employee?.rate || 0), status: 'published', location: '' }]);
  };
  ...
  const saveShiftLine = async (shift) => {
    try {
      ...
      if (shift.is_new) {
        await api.createSchedule({ employee_id: employee.id, client_id: client?.id || null, date: shift.date, start: normalizeTimeForInput(shift.start), end: normalizeTimeForInput(shift.end), pause: Number(shift.pause || 0), hours: Number(shift.hours || 0), km: Number(shift.km || 0), deplacement: Number(shift.deplacement || 0), autre_dep: Number(shift.other_dep || 0), notes: shift.notes || '', billable_rate: Number(shift.billable_rate || employee?.rate || 0), status: 'published', location: shift.location || '' });
      } else {
        await api.updateSchedule(shift.id, { start: normalizeTimeForInput(shift.start), end: normalizeTimeForInput(shift.end), pause: Number(shift.pause || 0), hours: Number(shift.hours || 0), km: Number(shift.km || 0), deplacement: Number(shift.deplacement || 0), autre_dep: Number(shift.other_dep || 0), notes: shift.notes || '' });
      }
  ...
  const saveQuickAccommodation = async () => {
    ...
    try {
      ...
      await api.createAccommodation({ employee_id: employee.id, total_cost: Number(accomForm.total_cost || 0), start_date: accomForm.start_date, end_date: accomForm.end_date, days_worked: 0, cost_per_day: 0, notes: accomForm.notes || '' });
  ...
  const totals = useMemo(() => {
    const service = editableShifts.reduce((sum, s) => sum + Number(s.hours || 0) * effectiveRate, 0);
    const km = editableShifts.reduce((sum, s) => sum + Number(s.km || 0) * RATE_KM, 0);
    const dep = editableShifts.reduce((sum, s) => sum + Number(s.deplacement || 0) * effectiveRate, 0);
