...
import WeeklyValidationQueue from '../components/WeeklyValidationQueue';
...
export default function SchedulesPage({ toast, onNavigate }) {
  ...
  const [validationQueue, setValidationQueue] = useState({ items: [], counts: { eleve: 0, moyen: 0, faible: 0 } });
  ...
  const currentWeekStart = useMemo(() => (viewMode === 'week' && viewDates.length >= 7 ? fmtISO(viewDates[0]) : null), [viewMode, viewDates]);
  const currentWeekEnd = useMemo(() => (viewMode === 'week' && viewDates.length >= 7 ? fmtISO(viewDates[6]) : null), [viewMode, viewDates]);
  ...
  const getValidationSummary = (employeeId, clientId) => (validationQueue?.items || []).find(item => item.employee_id === employeeId && item.client_id === clientId) || null;

  useEffect(() => {
    let cancelled = false;
    const loadValidationQueue = async () => {
      if (viewMode !== 'week' || !currentWeekStart) {
        setValidationQueue({ items: [], counts: { eleve: 0, moyen: 0, faible: 0 } });
        return;
      }
      try {
        setValidationLoading(true);
        const queue = await api.getWeeklyValidationQueue({ week_start: currentWeekStart });
        if (!cancelled) {
          setValidationQueue(queue || { items: [], counts: { eleve: 0, moyen: 0, faible: 0 } });
        }
      } catch (err) {
        if (!cancelled) {
          setValidationQueue({ items: [], counts: { eleve: 0, moyen: 0, faible: 0 } });
          toast?.('Erreur file validation: ' + err.message);
        }
      } finally {
  ...
  }, [viewMode, currentWeekStart, schedules, approvals, toast]);
